...
const FORMAT_OPTIONS = [
  {
    value: "testimoni",
    label: "📣 Testimoni",
    desc: "Review jujur dari pengalaman pribadi",
  },
  {
    value: "curiosity",
    label: "🤔 Create Curiosity",
    desc: "Hook bikin penasaran, reveal belakangan",
  },
  {
    value: "pengalaman",
    label: "📖 Cerita Pengalaman",
    desc: "Personal journey, produk sebagai solusi",
  },
  {
    value: "ketemu_barang",
    label: "✨ Ketemu Barang Baru",
    desc: "Discovery story, tone excited tapi genuine",
  },
  {
    value: "viral",
    label: "🔥 Barang Viral",
    desc: "Honest take soal barang yang lagi hype",
  },
  {
    value: "tip_life",
    label: "💡 Tips Kehidupan",
    desc: "Tips/hack, produk sebagai enabler",
  },
];
...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Gagal generate thread");
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-rose-50">
      ...
      <header className="border-b border-orange-100 bg-white/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-gray-900 leading-none">Shopee Thread Generator</h1>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-transparent resize-none"
            />
          ...
          <div>
            ...
            {form.ctaType === "inline" && (
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-transparent"
              />
            )}
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-3.5 bg-gradient-to-r from-orange-400 to-rose-500 text-white font-semibold rounded-xl transition-all hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed text-sm"
          >
            {loading ? (
          ...
          <div className="space-y-3">
            ...
            {tweets.map((tweet, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 group"
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className={`shrink-0 text-xs px-2.5 py-1.5 rounded-lg font-medium transition-all opacity-0 group-hover:opacity-100 ${
                        copied === idx
                          ? "bg-green-100 text-green-700"
            ...
            <button
              onClick={handleGenerate}
              className="w-full py-3 border-2 border-dashed border-gray-200 text-gray-500 text-sm font-medium rounded-xl hover:border-orange-300 hover:text-orange-500 transition-all"
            >
              🔄 Generate ulang
            </button>
          </div>
